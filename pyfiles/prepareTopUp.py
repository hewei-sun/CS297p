...
def initialTop100(url):  # Only called at Day 1
    ...
    for item in itemList:
        if item.find('a'):
            ...
            sql = mysqlconnect.getInsertToTable1Sql('PossibleTopUp', upID, numFollowings, numFollowers, numLikes,
                                                    numViews)
...
def updateUpByDate(upID, date):
    ...
    if rank != 0:
        ...
        (rank) = mysqlconnect.queryOutCome(sql2)[0][0]
    sql = '''INSERT IGNORE INTO `{}` VALUES ('{}', {}, {}, {}, {},{});'''.format('Up' + str(upID), date, numFollowings,
                                                                                 numFollowers, numLikes, numViews, rank)
    print(sql)
    mysqlconnect.queryOutCome(sql)
